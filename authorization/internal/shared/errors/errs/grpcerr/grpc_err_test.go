...
func TestErr_GRPXErr_ErrorsWithDetails(t *testing.T) {
	...
	errD, err := grpcerr.ErrorsWithDetails(&errauthzproto.ErrorDetail{Message: "This very very very very bad error"}, ":((", codes.InvalidArgument)
	assert.NoError(t, err)

	assert.Equal(t, 1, len(errD.Details()))
	errDetails, ok := errD.Details()[0].(*errauthzproto.ErrorDetail)
	assert.True(t, ok)
